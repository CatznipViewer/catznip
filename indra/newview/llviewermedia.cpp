...
// [SL:KB] - Patch: UI-FloaterInstance | Checked: Catznip-4.2
void LLViewerMedia::setOpenIDCookie(LLMediaCtrl* media_instance)
{
	std::string authority = sOpenIDURL.mAuthority;
	std::string::size_type hostStart = authority.find('@');
	if (hostStart == std::string::npos)
...
// [SL:KB] - Patch: Control-FilePicker | Checked: 2012-08-21 (Catznip-3.3)
			...
			{
				LLFilePicker::getOpenFile(LLFilePicker::FFLOAD_ALL, boost::bind(&LLViewerMediaImpl::onFilePickerCallback, plugin, _1));
			}
			else
			{
				LLFilePicker::getMultipleOpenFiles(LLFilePicker::FFLOAD_ALL, boost::bind(&LLViewerMediaImpl::onFilePickerMultipleCallback, plugin, _1));
			}
// [/SL:KB]
...
void LLViewerMediaImpl::onFilePickerMultipleCallback(LLPluginClassMedia* plugin, const std::vector<std::string>& filenames)
{
	std::vector<std::string> responses;
