...
void LLPanelObject::objectFromLLSD(const std::string& strParamType, const LLSD& sdParams)
{
	...
	else if ("rotation" == strParamType)
	{
		...
		rotEuler *= RAD_TO_DEG;
		rotEuler.mV[VX] = fmod(ll_round(rotEuler.mV[VX], OBJECT_ROTATION_PRECISION) + 360.f, 360.f);
		rotEuler.mV[VY] = fmod(ll_round(rotEuler.mV[VY], OBJECT_ROTATION_PRECISION) + 360.f, 360.f);
		rotEuler.mV[VZ] = fmod(ll_round(rotEuler.mV[VZ], OBJECT_ROTATION_PRECISION) + 360.f, 360.f);
