...
const static boost::regex TIMESTAMP("^(\\[\\d{4}/\\d{1,2}/\\d{1,2}\\s+\\d{1,2}:\\d{2}(:\\d{2})?\\]|\\[\\d{1,2}:\\d{2}(:\\d{2})?\\]).*");
// [/SL:KB]
...
bool LLLogChat::loadChatHistory(const std::string& file_name, std::list<LLSD>& messages, const LLSD& load_params, bool is_group)
{
	if (file_name.empty())
	{
