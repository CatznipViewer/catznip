...
// [SL:KB] - Patch: Chat-Logs | Checked: 2011-08-25 (Catznip-3.2.0a) | Added: Catznip-2.4.0c
	...
	{
		// NOTE-Catznip: [SL-2.6.0] mName will be:
...
bool LLIMModel::buildIMP2PLogFilename(const LLUUID& idAgent, const std::string& strName, std::string& strFilename)
{
	...
	LLAvatarName avName;
	if ( (LLAvatarNameCache::get(idAgent, &avName)) && (!avName.isTemporaryName()) )
	{
		if (!fLegacyFilenames)
		{
			// If display names are turned off mUserName will be empty and we have to construct it from the legacy name
			strFilename = (!avName.getAccountName().empty()) ? avName.getAccountName() : LLCacheName::buildUsername(avName.getDisplayName());
		}
		else
		{
