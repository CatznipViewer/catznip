...
{
	return normalize(mName, mSize, mStyle, &mFileNames, &mFontCollectionsList);
}
// [/SL:KB]
...
// [SL:KB] - Patch: UI-Font | Checked: 2014-02-27 (Catznip-3.6)
// static
LLFontDescriptor LLFontDescriptor::normalize(const std::string& name, const std::string& size, const U8 style, const string_vec_t* file_names_p, const string_vec_t* font_collections_p)
{
	...
	if ( (file_names_p) || (font_collections_p) )
		return LLFontDescriptor(new_name, new_size, new_style, (file_names_p ) ? *file_names_p : string_vec_t(), (font_collections_p ) ? *font_collections_p : string_vec_t());
